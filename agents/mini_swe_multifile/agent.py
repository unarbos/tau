#!/usr/bin/env python3
"""
Multi-file SWE coding agent for the tau subnet, ported from the full
mini-swe-agent (https://github.com/SWE-agent/mini-swe-agent) default agent.
...
Layout:
    ...
    mini_swe/prompts.py  system/instance templates tuned for diff-match scoring
    mini_swe/model.py    stdlib OpenAI-compatible chat client with retries
    mini_swe/environment.py  fresh-subshell bash executor
    mini_swe/agent_loop.py   the query -> act -> observe step loop
    mini_swe/repo_diff.py    harness-compatible patch collection
...
from mini_swe.agent_loop import AgentRunConfig, run_mini_swe_agent
from mini_swe.prompts import build_task_prompt
from mini_swe.repo_diff import collect_repo_patch
