"""The mini-swe-agent step loop: query the model, run one bash action, feed
the observation back, finish when the agent echoes the completion sentinel.
Ported from mini-swe-agent's DefaultAgent with a text-based action format."""
...
def run_mini_swe_agent(*, config: AgentRunConfig, task: str) -> AgentOutcome:
    model = ChatModel(
